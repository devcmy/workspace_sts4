...
②HandlerInterceptorAdapter 추상클래스
	...
	- public boolean postHandle(HttpServletRequest request, HttpServletResponse response,Object handler, ModelAndVeiw modelAndVeiw)
     	Controller 처리가 끝나고 화면에 띄어주는 처리 직전에 수행 -- 후
	
	- afterCompletion() 
	    Controller 모든 처리가 끝난 후 호출 --완전히 끝날떄
 */
...
 
			디스패처 서블릿은 애플리케이션이 실행될 때 모든 컨트롤러 빈의 메소드를 살펴서(Reflection)해서 
			매핑 후보가 되는 메소드들을 추출한 뒤, 이를 HandlerMethod 형태로 만든다음에 url와 mapping을 저장해둔다. 
		...
		*************************************************************************************/   
		if (handler instanceof HandlerMethod == false) { //핸들러 메소드 타입이니? 아니면(false)면 컨트롤러로 직행 맞으면, 로그인체크 처럼 필요한것을 체크함.
			/*
