...
public class JSONMessageConverterRestController {
	...
	//restcontroller에서 객체가 반환되면, 컨버터를 찾아서 보내줌
	
	@GetMapping(value="/news/{no}", produces = "application/json;charset=UTF-8") //{no} = path variable, parameter처럼 받을수있음.
	public News newsTitleJSON(@PathVariable int no) { //@PathVariable(name = "no")  name="no" 생략
